...
import { Package, User, LogOut, X, Menu } from "lucide-solid";
...
export default function TopNav(props: TopNavProps) {
  ...
  const linkClasses = (path: string) => {
    const baseClasses =
      "border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700 inline-flex items-center px-1 border-b-2 text-sm font-medium";
    const isActive =
      path === "/dashboard"
  ...
  return (
    <nav class="bg-white  top-0 left-0 w-full z-50 px-4 lg:px-12">
      <div class="flex justify-between h-16 items-center ">
        <div class="flex items-center ">
          <A href="/dashboard" class="flex items-center">
            <YourLogo class="h-4 md:h-6 w-auto text-black" />
          </A>
        </div>
